...
import type { PlasmoCSConfig, PlasmoGetInlineAnchorList, PlasmoCSUIAnchor } from "plasmo"
...
function AddNotesBtn({ onClick }: AddNotesBtnProp) {
  return (
    <button type="button" className="px-3 py-1 text-xs font-medium text-center inline-flex items-center rounded-lg cursor-pointer text-blue-700 hover:bg-blue-700 hover:text-white" onClick={onClick}>
...
function AddIcon({ isEmbedded = false, onClick }: AddIconProp) {
  ...
  if (!isEmbedded) {
    wrapperClass = "text-gray-400 hover:text-lime-900 hover:bg-lime-100 cursor-pointer p-1 rounded-full"
  }
...
function TrashIcon({ onClick }: TrashIconProp) {
  return (
    <div className="text-gray-400 hover:text-rose-900 hover:bg-rose-100 cursor-pointer p-1 rounded-full" onClick={onClick}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    ...
    <div className="flex items-center mb-4">
      <div className="w-11/12 rounded-md bg-blue-50 p-4 flex items-center mr-4">
        ...
        <div className="ml-3 flex flex-1 justify-between">
          ...
          <p className="text-sm text-blue-700 ml-6 font-medium text-center">{timeAgo.format(new Date(note.createdAt))}</p>
        </div>
      </div>
  ...
  return (
    <form action="#" className="relative w-full mb-4">
      <div className="overflow-hidden rounded-lg ring-blue-700 ring-1 ring-inset shadow-sm">
        <textarea rows={3} name="note" className="block w-full border-0 bg-transparent py-1.5 text-sm leading-6 resize-none" placeholder="Add a note" value={content} onChange={(e) => setContent(e.target.value)} autoFocus></textarea>
        <div className="py-2">
          <div className="py-px">
      ...
      <div className="absolute left-0 right-0 bottom-0 flex justify-between py-2 pl-3 pr-2">
        <button type="button" className="rounded-md bg-transparent px-3 py-2 text-sm font-semibold hover:bg-blue-700 hover:text-white" onClick={onCancel}>Cancel</button>
        <button type="button" className="rounded-md bg-blue-700 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500" onClick={() => onSave(content)}>Save</button>
      </div>
    </form>
  )
}
...
function NotesManagerUIPrimary({ anchor }: { anchor: PlasmoCSUIAnchor }) {
  ...
  const itemName = useMemo(() => anchor.element.querySelector(':scope h3').textContent, [anchor])
  ...
  if (notes.length === 0 && !showNotesInput) {
    addNotesBtn = (
      <AddNotesBtn onClick={() => setShowNotesInput(true)} />
    )
  }
  ...
  return (
    <div className="plasmo-shield">
      {addNotesBtn}
      {
        notes.map((n, i) => <NoteItem note={n} key={n.id} onDelete={onDelete} showAddBtn={i === notes.length - 1} onAdd={() => setShowNotesInput(true)} /> )
      }
      {addNotesInput}
    </div>
  )
