...
function NotesManagerUISecondary({ anchor }: { anchor: PlasmoCSUIAnchor }) {
  const nodes = [...document.querySelectorAll('[data-testid="normal-dish-item"] h3')]
  const item = nodes.filter(n => n.textContent === anchor.element.textContent )[0]
  ...
  return (
    <div className="absolute -top-9 right-0 cursor-pointer opacity-50 hover:opacity-100" onClick={scrolToItem}>
      <NotesIcon />
    </div>
  )
}
